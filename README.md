...
- ✅ Axios API integration with JWT tokens
- ✅ Beautiful gradient UI

### Backend (Express.js)
- ✅ JWT token validation middleware
- ✅ Todo CRUD API endpoints
- ✅ User-specific data isolation (using `req.auth.sub`)
...
### Admin Only
- `GET /api/admin/all-todos` - Get all todos from all users (requires admin role)
...
### Test Todo CRUD
1. Create a todo using the form
2. Mark todos as complete
3. Delete todos
4. All operations require JWT authentication

### Test RBAC (Admin Access)
To test admin endpoints, you need to:
1. Create an `admin` role in Asgardeo
2. Assign it to a user
3. The JWT token will include `"groups": ["admin"]`
4. Access: `GET http://localhost:4000/api/admin/all-todos`

## 📚 Documentation

See `TODO_APP_GUIDE.md` for:
...
│   ├── src/
│   │   ├── components/
│   │   │   └── TodoList.jsx         # Todo CRUD component
│   │   ├── pages/
│   │   │   ├── SignIn.jsx           # Login page
...
│
└── backend/
    ├── server.js                    # Express API + JWT validation
    ├── .env                         # Asgardeo credentials
    └── package.json
```
...
✅ **JWT Token Validation** - All API routes are protected  
✅ **User Data Isolation** - Users see only their own todos  
✅ **Role-Based Access Control** - Admin endpoints require admin role  
✅ **Token Expiration** - Automatic validation of token expiry  
✅ **Signature Verification** - Ensures tokens are from Asgardeo  
...
2. **Add Refresh Tokens** - Implement token refresh logic
3. **Deploy** - Deploy to Vercel (frontend) + Railway (backend)
4. **Add Tests** - Write unit tests for API endpoints
5. **Enhance UI** - Add loading states, error boundaries, etc.
...
### Admin endpoint returns 403
- User must have `admin` role in Asgardeo
- Check JWT token includes `groups: ["admin"]`
- Configure Asgardeo to include groups in token

## 📞 Support

For detailed explanations, see:
- `TODO_APP_GUIDE.md` - Complete implementation guide
- [Asgardeo Docs](https://wso2.com/asgardeo/docs/)
- [Express JWT Docs](https://github.com/auth0/express-oauth2-jwt-bearer)
